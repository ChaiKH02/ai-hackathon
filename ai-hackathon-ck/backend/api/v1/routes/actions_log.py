...
async def get_all_actions(
    ...
    last_evaluated_key_dict = response.get("LastEvaluatedKey", None)

    # Encode next_token if key exists
    new_next_token = None
    if last_evaluated_key_dict:
        import base64
        import json
        ...
        token_json = json.dumps(last_evaluated_key_dict, cls=DecimalEncoder)
        new_next_token = base64.b64encode(token_json.encode("utf-8")).decode("utf-8")

    # --- Get Total Count (Separate Scan) ---
    count_kwargs = {
        "Select": "COUNT"
    }
    if filter_expression:
