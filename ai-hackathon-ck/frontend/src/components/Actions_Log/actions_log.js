...
const ActionsLog = () => {
    ...
    // Fetch Actions
    const fetchActions = async (token = null) => {
        setLoading(true);
        try {
    ...
    // Initial fetch & Filter change reset
    useEffect(() => {
        setHistory([]);
        setNextToken(null);
    ...
    const handleNext = () => {
        if (nextToken) {
            setHistory([...history, currentPageToken]);
            fetchActions(nextToken);
        }
    };
        ...
        <div className="actions-log-container">
            <div className="header-actions" style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', background: 'var(--bg-light)', padding: '0.2rem 0.6rem', borderRadius: '20px' }}>
                        Total: {totalCount}
                    </span>
                </div>
            ...
            {loading ? (
                ...
                <>
                    ...
                    {/* Pagination Controls */}
                    <div className="pagination-controls" style={{ marginTop: '2rem', display: 'flex', justifyContent: 'center', gap: '1rem', alignItems: 'center' }}>
                        <button
                            onClick={handlePrev}
                            ...
                            style={{
                                ...
                                borderRadius: '6px',
                                background: history.length === 0 ? 'var(--bg-light)' : 'white',
                                color: history.length === 0 ? 'var(--text-secondary)' : 'var(--text-main)',
                        ...
                        <button
                            ...
                            style={{
                                ...
                                border: '1px solid var(--border)',
                                borderRadius: '6px',
                                background: !nextToken ? 'var(--bg-light)' : 'white',
                                color: !nextToken ? 'var(--text-secondary)' : 'var(--text-main)',
                                cursor: !nextToken ? 'not-allowed' : 'pointer',
                                display: 'flex', alignItems: 'center', gap: '0.5rem'
                            }}
                        >
                            Next <span>&rarr;</span>
